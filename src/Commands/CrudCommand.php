...
class CrudCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->call('crud:controller', ['name' => $controllerNamespace . str_singular($name) . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
        ...
        File::put($pathToPathsFile, str_replace('];', '"' . lcfirst($name) . '" => "/uploads/' . lcfirst($name) . '",' . "\n" . '];', File::get($pathToPathsFile)));
